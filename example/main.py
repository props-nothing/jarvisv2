...
TOOL_DECLARATIONS = [
    ...
    },
    {
        ...
        "description": (
            ...
            "On a multi-monitor machine the capture is labelled with which screen it "
            "shows (and 'ALL SCREENS' when every display was grabbed); trust that "
            "label over your own assumption about which screen you are looking at. "
        ...
        "parameters": {
            ...
            "properties": {
                ...
                "screens": {
                    "type": "STRING",
                    "description": (
                        "Which display to capture when angle is 'screen': "
                        "'active' (default) = the whole screen holding the window the "
                        "user is working in; 'window' = just that window, cropped "
                        "(more detail for small text); 'all' = every monitor as one "
                        "image, use when the user asks about all their screens or you "
                        "do not know which one; or a monitor number (1, 2, ...)."
                    )
                }
            },
            "required": ["text"]
...
def _is_1008(err_str: str) -> bool:
    """True for the websocket 1008 'policy violation' close.

    Seen live as three consecutive failures right after the startup briefing,
    surfacing as `APIError: 1008 None. The operation was aborted.` wrapped in an
    ExceptionGroup by the TaskGroup. The code is the only stable part of the
    message — the reason text is empty — so match on the code and the close
    phrase rather than the wording around them.
    """
    s = err_str or ""
    return ("1008" in s
            or "policy violation" in s.lower()
            or "operation was aborted" in s.lower())
...
class JarvisLive:
    ...
    def _build_config(self) -> types.LiveConnectConfig:
        ...
        # How many displays there are, and where they sit. Without this the model
        # has no way to know a second monitor exists, and says "your screen"
        # about a desktop spanning two of them — or confidently places an
        # element on the wrong one.
        _screens_desc = ""
        try:
            from core import screens as _screens
            _screens_desc = _screens.describe()
        except Exception:
            pass
            ...
            if name == "recall_memory":
                ...
                if self._vision_busy or (_now - self._vision_last_time) < _cooldown:
                    ...
                    if angle == "camera":
                        ...
                        _target = args.get("screens") or args.get("monitor") or "active"
                        img_b, mime_t = await loop.run_in_executor(
                            None, lambda: _capture_screen(_target)
                        )
        ...
        while True:
            try:
                ...
                async with (
                    ...
                    # A successful connect clears the failure streak, so the fast
                    # 3s retry is available again after any real outage. This is
                    # what stops the backoff from climbing across a whole session.
                    self._fail_streak = 0
                    if _resumed_with:
                ...
                # Escalating backoff for ANY repeated failure.
                #
                # The bug this fixes: the branch below reset the delay to a flat
                # 3s for every error that was not a network error. A persistent
                # 1008 (policy violation) therefore reconnected every three
                # seconds forever — visible in the log as three failures in a row
                # after the startup briefing, each one costing a fresh session
                # and a fresh briefing. Nothing about that loop ever slows down
                # or gives up, so a configuration problem becomes an infinite
                # hammering of the API.
                #
                # A consecutive-failure streak fixes it without needing to know
                # which errors are permanent: the first retry is as fast as it
                # ever was, and only a REPEATED failure earns a longer wait.
                self._fail_streak = getattr(self, "_fail_streak", 0) + 1
                if self._fail_streak >= 5:
                    self._conn_backoff = min(30 * (self._fail_streak - 3), 300)
                elif self._fail_streak >= 2:
                    self._conn_backoff = min(3 * (2 ** (self._fail_streak - 1)), 60)
                if self._fail_streak == 5:
                    self.ui.write_log(
                        "SYS: Repeated connection failures — backing off. "
                        "Check the API key and quota."
                    )
                ...
                if "API key not valid" in err_str or "1007" in err_str:
                    ...
                    # A new key is a genuinely new attempt: clear the streak so
                    # the retry is immediate rather than inheriting the previous
                    # key's backoff.
                    self._fail_streak = 0
                ...
                if is_net_err:
                    ...
                    # Do NOT flatten this back to 3s. The escalating value set
                    # above — which counts consecutive failures — stands, so a
                    # server-side error that keeps recurring slows down instead
                    # of reconnecting every three seconds forever. Only a first
                    # failure gets the old immediate-ish retry.
                    if self._fail_streak < 2:
                        self._conn_backoff = 3
                    if _is_1008(err_str):
                        self.ui.write_log(
                            "SYS: The server closed the session (1008). Backing off."
                        )
            finally:
                self.session = None
